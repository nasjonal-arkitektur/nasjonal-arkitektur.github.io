...
Fra et arkitekturståsted så består en referansearkitektur av et sett med funksjonelle byggeklosser (ABBer) som representerer funksjonaliteten som kreves for å realisere samhandlingsbehovet. Det vil både kunne være påbudte og valgfrie funksjonelle byggeklosser. Valgfrie byggeklosser benyttes kun der behov og krav til kvalitetsegenskaper tilsier at det kreves. For eksempel vil ikke alle samhandlingsbehov kreve kapabiliteten meldingskryptering for link:../na_arkitekturbibliotek_referanse_arkitekturer_eoppslag/eOppslag-referansearkitektur.adoc[eOppslag].

Hver av de funksjonelle byggeklossene kan realiseres av en eller flere løsningskomponenter (SBBer). En funksjonell byggekloss kan ha flere mulige løsningskomponenter. Rett løsningskomponent velges i så fall basert på behov og kvalitetsegenskapene som kreves og som best svarer på behov og krav til kvalitetsegenskaper, inkludert behov avdekket som del av risikoanalyse for sikkerhet og personvern. Se følgende konseptuelle figur:

image:./images/mapping-referansearkitektur-til-løsningsarkitektur.png[alt="missing figure", width=500]
...
* link:../na_arkitekturbibliotek_referanse_arkitekturer_eoppslag/eOppslag-referansearkitektur.adoc[eOppslag] 
